...
namespace R61M11C03_w01.Controllers
{
    ...
    public class SalesController : ControllerBase
    {
        ...
        public SalesController(ProductContext db, IWebHostEnvironment webHost)
        {
            this.db = db;
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<SalesMaster>>> GetOrders()
        {
            var orders = await db.SalesMasters
                                   .Include(a => a.SalesDetails)
                                   .ThenInclude(d => d.Product)
                                   .Include(c=>c.Customer)
                                   .ToListAsync();

            var orderDtos = orders.Select(p => new SalesMaster
            {
                Id = p.Id,
                ...
                SalesDetails = p.SalesDetails.Select(a => new SalesDetails
                {
                    Id = a.Id,
                    Price = a.Price,
                    SalesID = a.SalesID,
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<SalesMaster>> Get(int id)
        {
            var p = await db.SalesMasters.Include(p => p.SalesDetails).ThenInclude(a => a.Product).FirstOrDefaultAsync(p => p.Id == id);
            if (p == null) return NotFound();
            var data = new SalesMaster
            {
                Id = p.Id,
                CustomerName = p.Customer.Name,
                OrderDate = p.OrderDate,
        ...
        public IActionResult Post()
        {
            ...
            var p = HttpContext.Request.Form["orderdata"];
            var entity = JsonConvert.DeserializeObject<SalesMaster>(p);
            try
            {
                ...
                if (requestedFile != null)
                {
                    ...
                    var sale = new SalesMaster
                    {
                        ...
                        SalesDetails = entity.SalesDetails,
                       // Picture = "/Pictures/" + fileName

                    };
                    db.SalesMasters.Add(sale);
                    db.SaveChanges();
        ...
        public IActionResult Put()
        {
            var p = HttpContext.Request.Form["orderdata"];
            var entity = JsonConvert.DeserializeObject<SalesMaster>(p);
            var existing = db.SalesMasters.Find(entity.Id);
            ...
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    existing.SalesDetails = entity.SalesDetails;
        ...
        //}
        [HttpDelete("{id}")]
        public async Task<ActionResult<SalesMaster>> Delete(int id)
        {
            var p = await db.SalesMasters.Include(p => p.SalesDetails).FirstOrDefaultAsync(p => p.Id == id);
            if (p == null) return NotFound();
            db.SalesMasters.Remove(p);
            if (db.SaveChanges() > 0)
            {
                return Ok();
            }
